...
export const getRankTrackerTool = {
  ...
  config: {
    ...
    description:
      "Read-only access to rank tracker configs and their latest results. With `trackerId`, returns config + latest snapshot per keyword. Without it, lists all trackers in the project. Uses no credits — reads from TheGoodSite SEO state, no DataForSEO call. To trigger a new check, use the dashboard.",
    inputSchema,
    outputSchema: z
      .object({
